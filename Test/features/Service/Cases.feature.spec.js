...
test.describe('Cases', () => {
  ...
  test('Create, Close and Delete a case', { tag: ['@service', '@cases'] }, async ({ Given, page, pages, scenarioContext, When, And, Then }) => { 
    ...
    await And('user "creates" the case', {"dataTable":{"rows":[{"cells":[{"value":"Account Name"}]},{"cells":[{"value":"Account"}]}]}}, { pages, scenarioContext }); 
    await And('user changes the case status to "Working"', null, { pages }); 
    await And('user changes the case status to "Waiting on Customer"', null, { pages }); 
    await And('user changes the case status to "Escalated"', null, { pages }); 
    await And('user changes the case status to "Closed"', null, { pages }); 
